...
def test_staleness_gate_stays_in_in_progress_for_dwell_then_invalidates_with_staleness_expired(monkeypatch) -> None:
    ...
    stale_metadata["detected_at_wallclock"] = (now - timedelta(minutes=65)).isoformat()
    setup = replace(setup, created_at=now, completed_at=now - timedelta(minutes=65), metadata=stale_metadata)  # already past STALE_ENTRY_READY_MAX_AGE
