...
from arjiobot.risk.risk_models import OpenRiskState, RiskConfig, RiskRejectionReason
...
def reasons_for(signal, config=None, state=None):
    config_, snapshot, state_ = default_context()
    return validate_signal_risk(signal=signal, risk_config=config or config_, account_snapshot=snapshot, open_risk_state=state or state_)[0]
...
def test_rr_too_low_and_leverage_exceeds() -> None:
    signal = make_signal()
    assert RiskRejectionReason.RR_TOO_LOW in reasons_for(signal, config=risk_config(minimum_rr_ratio=Decimal("2")))
    assert RiskRejectionReason.LEVERAGE_EXCEEDS_MAX in reasons_for(signal, config=risk_config(allocated_margin=Decimal("1"), max_leverage=Decimal("2")))
