  ...
  return (
    <DataTable
      ...
      columns={[
        ...
        { header: 'Completed %', render: (row) => `${(row.progress_pct ?? row.progress_percent).toFixed(0)}%` },
        { header: 'Last Valid Stage', render: (row) => <StatusBadge label={friendlyStageLabel(row.last_valid_stage ?? row.current_stage ?? row.current_state, row.direction)} tone={statusTone(row.status)} /> },
