...
export function SetupRadarTable({ setups, onSelect }: { setups: RadarSetup[]; onSelect?: (setup: RadarSetup) => void }) {
  ...
  return (
    <DataTable
      ...
      columns={[
        { header: 'Symbol', render: (row) => <button className="text-action" onClick={() => onSelect?.(row)}>{row.symbol}</button> },
        ...
        { header: 'Invalidation', render: (row) => row.rejection_reason ?? row.invalidation_reason ?? '-' },
